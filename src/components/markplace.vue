  ...
  export default {
    ...
    methods: {
      ...
      getcmt(comment_id){
        ...
        this.axios.get('api/comments/',config1)
          .then(function (response) {
            ...
            for (var i=0;i<this.current_tag_id_list.length;i++) {
              if (this.current_tag_id_list[i]){
                this.axios.get('api/tag/', {params: {tag_id: this.current_tag_id_list[i]}})
                  .then(function (response) {
      ...
      saveMark(key) {
        ...
        if (this.current_tag === "") {
          this.axios.post('api/tag/', formData1)
            .then(function (response) {
              console.log(response)
          ...
          this.axios.put('api/tag/', formData2)
            .then(function (response) {
              console.log(response)
              if (response.data.Msg === "OK") {
                this.issave = true;
